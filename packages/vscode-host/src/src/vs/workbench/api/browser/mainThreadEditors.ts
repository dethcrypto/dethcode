/**
 * Changes introduced in this file:
 * * tryShowTextDocument - hides welcome page automatically
 */
...
export class MainThreadTextEditors implements MainThreadTextEditorsShape {
	...
	async $tryShowTextDocument(resource: UriComponents, options: ITextDocumentShowOptions): Promise<string | undefined> {
		...
		// Auto close "Getting Started" page if any other document gets opened
		try {
			await this._editorService.findEditors(GettingStartedInput.RESOURCE).forEach(editor => this._editorService.closeEditor(editor));
		}
		catch (e) {
			console.error("Error occured while trying to close Getting started", e);
		}
